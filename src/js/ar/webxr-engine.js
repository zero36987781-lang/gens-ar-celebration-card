...
export class WebXREngine {
  ...
  buildGiftMeshes() {
    ...
    const edgeMaterial = new THREE.MeshStandardMaterial({ color: 0xe7edf8, roughness: 0.68, metalness: 0.14 });
    this.cardMesh = new THREE.Mesh(
      new THREE.BoxGeometry(0.62, 0.88, 0.03),
      [edgeMaterial, edgeMaterial, edgeMaterial, edgeMaterial, new THREE.MeshStandardMaterial({ map: frontTexture }), new THREE.MeshStandardMaterial({ map: backTexture })]
    );
    ...
    this.floorCircle = new THREE.Mesh(
      new THREE.CircleGeometry(0.07, 28).rotateX(-Math.PI / 2),

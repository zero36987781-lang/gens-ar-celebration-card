...
import { createRecipientUrl, generateSlug, getCurrentPosition, qs, qsa, readFileAsDataURL, setStatus, toIsoFromHours } from '../core/utils.js';
...
async function handleSubmit(event) {
  ...
  try {
    ...
    setStatus(
      els.statusBox,
      `Saved. Link expires at ${new Date(saved.expiresAt).toLocaleString()}. Placement defaults: ${Number(saved.spawnHeight || 1.6).toFixed(1)}m high / ${Number(saved.forwardDistance || 1.6).toFixed(1)}m forward.`,
      'success'
    );
  } catch (saveError) {

...
function nextPage() { if (state.page < MAX_PAGES) { state.page++; updatePage(); qs('.sender-shell')?.scrollTo({ top: 0, behavior: 'smooth' }); } }
function prevPage() { if (state.page > 1) { state.page--; updatePage(); qs('.sender-shell')?.scrollTo({ top: 0, behavior: 'smooth' }); } }

/* ── Studio split resizer ── */
function bindStudioResizer() {
  const divider = els.studioDivider;
  const canvas = els.studioCanvasArea;
  if (!divider || !canvas) return;

  const doResize = (clientY) => {
    const parentRect = canvas.parentElement.getBoundingClientRect();
    const newH = clientY - parentRect.top;
    const minH = 150;
    const maxH = parentRect.height - 120;
    canvas.style.height = Math.max(minH, Math.min(maxH, newH)) + 'px';
    if (window.CanvasEditor) window.CanvasEditor.resizeStages();
  };

  divider.addEventListener('mousedown', e => {
    e.preventDefault();
    const mm = ev => doResize(ev.clientY);
  ...
  divider.addEventListener('touchstart', e => {
    e.preventDefault();
    const tm = ev => { ev.preventDefault(); doResize(ev.touches[0].clientY); };
...
function syncSideToggle() {
  ...
  [els.previewCardDefault, els.previewCard].forEach(card => {
    ...
    card.classList.toggle('is-front', !showBack);
  });
...
function closeAllPanels() {
  ...
  document.querySelectorAll('.icon-tb-btn').forEach(b => b.classList.remove('active'));
}
...
function renderVideoPreview() {
  ...
  if (!url) { els.videoPreviewArea.innerHTML = '<div class="video-preview-empty">Paste a YouTube URL.</div>'; els.videoPreviewStatus.textContent = 'YouTube only.'; renderPreviewCard(); return; }
  if (!ytId) { els.videoPreviewArea.innerHTML = '<div class="video-preview-empty">Only YouTube URLs.</div>'; els.videoPreviewStatus.textContent = ''; renderPreviewCard(); return; }
  const vs = hmsToSec('vs'), ve = hmsToSec('ve'), ss = Number.isFinite(vs) ? vs : 0, se = Number.isFinite(ve) ? ve : ss + 12;
  const emb = new URL(`https://www.youtube.com/embed/${ytId}`);
  emb.searchParams.set('start', String(ss)); emb.searchParams.set('end', String(Math.max(ss + 1, se)));
  emb.searchParams.set('rel', '0'); emb.searchParams.set('playsinline', '1');
  ...
  els.videoPreviewStatus.textContent = `${secToHms(ss)} → ${secToHms(Math.max(ss + 1, se))}`;
  renderPreviewCard();
}
...
function syncEditorMode() {
  const isCustom = state.editorMode === 'custom';
  els.basicPreview?.classList.toggle('hidden', isCustom);
  els.customPreview?.classList.toggle('hidden', !isCustom);
  document.querySelectorAll('#mode-toggle .toggle-pill').forEach(p => p.classList.toggle('active', p.dataset.value === state.editorMode));
  if (els.iconToolbar) els.iconToolbar.style.display = isCustom ? '' : 'none';
  if (!isCustom) closeAllPanels();

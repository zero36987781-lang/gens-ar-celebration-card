const CanvasEditor = (() => {
  ...
  function cacheDom() {
    ...
    els.ctxOpacityPct = $('ctx-opacity-pct');
    els.ctxDup = $('ctx-dup');
    els.ctxDel = $('ctx-del');
  }
  ...
  function resizeStages() {
    [els.stageFront, els.stageBack].forEach(s => {
      if (!s) return;
      const parent = s.parentElement;
      if (!parent) return;
      const pw = parent.offsetWidth;
      const ph = parent.offsetHeight;
      const scale = Math.min(pw / 320, ph / 448);
      const rot = (s === els.stageBack) ? ' rotateY(180deg)' : '';
      s.style.transform = `scale(${scale})${rot}`;
      // Center it
      if (s === els.stageBack) {
        s.style.left = `${(pw + 320 * scale) / 2}px`;
      } else {
        s.style.left = `${(pw - 320 * scale) / 2}px`;
      }
      s.style.top = `${(ph - 448 * scale) / 2}px`;
    });
  }
  ...
  function resetDefaultLayers() {
    layers = {
      front: [
        { id:uid(),type:'text',text:'Happy Birthday',x:20,y:40,fontSize:28,opacity:1,fillType:'solid',fillColor:'#FFFFFF',strokeColor:'transparent',gradStops:null,gradAngle:135 },
        { id:uid(),type:'text',text:'A bright surprise for your special day.',x:20,y:80,fontSize:12,opacity:0.88,fillType:'solid',fillColor:'rgba(255,255,255,0.88)',strokeColor:'transparent',gradStops:null,gradAngle:135 },
        { id:uid(),type:'text',text:'Happy Birthday! Wishing you joy,\nlaughter, and a beautiful year ahead.',x:20,y:120,fontSize:13,opacity:1,fillType:'solid',fillColor:'#FFFFFF',strokeColor:'transparent',gradStops:null,gradAngle:135 },
        { id:uid(),type:'text',text:'From Sender',x:20,y:380,fontSize:12,opacity:0.9,fillType:'solid',fillColor:'rgba(255,255,255,0.9)',strokeColor:'transparent',gradStops:null,gradAngle:135 }
      ],
      back: [
        { id:uid(),type:'text',text:'Back side',x:20,y:30,fontSize:11,opacity:0.7,fillType:'solid',fillColor:'rgba(255,255,255,0.7)',strokeColor:'transparent',gradStops:null,gradAngle:135 },
        { id:uid(),type:'text',text:'Thank you for being such a\nspecial part of my life.',x:20,y:70,fontSize:14,opacity:1,fillType:'solid',fillColor:'#FFFFFF',strokeColor:'transparent',gradStops:null,gradAngle:135 }
      ]
    };
  }
  ...
  function bindCtxBar() {
    const updateOp = (val) => {
      ...
      if(els.ctxOpacityPct) els.ctxOpacityPct.textContent = Math.round(l.opacity * 100) + '%';
      renderLayers();
    };
    els.ctxOpacity?.addEventListener('input', e => updateOp(e.target.value));
    
    // Drag to adjust percentage on the label area
    let startX = 0, startVal = 0;
    els.ctxOpacityPct?.parentElement?.addEventListener('pointerdown', e => {
      startX = e.clientX;
      startVal = sel()?.opacity ?? 1;
      const mm = ev => {
        const delta = (ev.clientX - startX) / 200;
        const nv = Math.max(0, Math.min(1, startVal + delta));
        if(els.ctxOpacity) els.ctxOpacity.value = nv;
        updateOp(nv);
      };
  ...
  function showCtxBar() {
    ...
    const l = sel();
    if(l) { 
      els.ctxOpacity.value = l.opacity ?? 1; 
      if(els.ctxOpacityPct) els.ctxOpacityPct.textContent = Math.round((l.opacity ?? 1) * 100) + '%';
    }

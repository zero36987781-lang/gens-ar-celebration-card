...
function renderGift() {
  ...
  if (gift.videoUrl && !parseYouTubeId(gift.videoUrl)) {
...
function startPreviewAutomation() {
  ...
  window.setTimeout(() => handleBegin(), 500);
  window.setTimeout(() => checkDistance(), 1400);
  window.setTimeout(() => launchCardStage(), 2600);
}
